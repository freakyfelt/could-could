...
import { ResourcePolicyResolver } from "./resolver";
import { BasicContextualResourcePolicy } from "./__fixtures__/resource-policies";
...
describe("ResourcePolicyResolver", () => {
  const policyStore = createPolicyStore({
    policies: [BasicContextualResourcePolicy],
    targetEnvironment: "production",
  });
  const resolver = new ResourcePolicyResolver(policyStore);
