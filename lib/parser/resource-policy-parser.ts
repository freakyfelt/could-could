...
import type { ActionPolicyDefinition, JsonLogicParser, ResourcePolicyDocument } from '../types'
...
type ResourcePolicy = Map<string, RulesLogic>
...
function mapPoliciesToActions(policies: ActionPolicyDefinition[], actions: string[]): Map<string, ActionPolicyDefinition[]> {
    ...
    policies.forEach((policy) => {
      if (Array.isArray(policy.action)) {
        policy.action.forEach((action) => policiesByAction.get(action)!.push(policy))
      } else {
    ...
    policiesByAction.forEach((policies, key) => policiesByAction.set(key, [...policies, ...globalActions]))
...
export class ResourcePolicyParser {
  ...
  constructor(opts: ResourcePolicyParserOptions = {}) {
    this.policyValidator = opts.policyValidator ?? new ResourcePolicyValidator()
  }
  ...
  parse({ doc, targetEnvironment }: ParseInput): ResourcePolicy {
    ...
    const relevantPolicies = doc.definitions.filter((def) => {
      if (Array.isArray(def.environment)) {
        return def.environment.some((env) => ['*', targetEnvironment].includes(env))
      } else {
        return ['*', targetEnvironment].includes(def.environment)
      }
    }).flatMap((def) => def.policies)

    const policiesByAction = mapPoliciesToActions(relevantPolicies, doc.actions)
  ...
  private compileActionPolicies(policies: ActionPolicyDefinition[]): RulesLogic {
    ...
    const allow = toAllow.length === 0 ? false : {
      some: [
        toAllow,
    ...
    const deny = toDeny.length === 0 ? true : {
      none: [
        toDeny,
        // Check if any resolved value equals true
        { '===': [{ var: '' }, true ] }
